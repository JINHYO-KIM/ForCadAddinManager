# SmartInfraDevJH Development Notes
...
1. Build the **Core** project only (`JH_CadAddinManager_SmokeTest.dll`) — this is the assembly with all the `[CommandMethod]` logic and no ribbon.
2. Open CadAddinManager inside Civil3D and **Add** `JH_CadAddinManager_SmokeTest.dll`.
...
1. Build both projects: `JH_CadAddinManager_SmokeTest.dll` (logic) and `Civil3dAddinBridge_Shell.dll` (Shell, ribbon).
...
3. Register only the Shell assembly (`Civil3dAddinBridge_Shell.dll`) as the add-in to auto-load with Civil3D (via bundle/manifest, or NETLOAD for a quick check).
